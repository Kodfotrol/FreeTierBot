...
def process_message(msg):
    ...
    if text == "/start":
        ...
        send_message(chat_id, f"🎨 Генерирую изображение «{desc}»... (быстрая модель, до 5 сек)")
        try:
            ...
            cursor = conn.cursor()
            cursor.execute(
                "INSERT INTO generations (type, description, status, result) VALUES ('image', ?, 'done', ?)",
                (desc, img_path)
            )
            conn.commit()
            conn.close()
        ...
        send_message(chat_id, f"💻 Генерирую {cmd}: «{desc}»... Это займёт до минуты.")
        try:
            ...
            if len(code) > 1500:
                ...
                with open(file_path, "w", encoding="utf-8") as f:
                    f.write(code)
                send_message(chat_id, f"✅ *{cmd.upper()} «{desc}» готов!*", parse_mode="Markdown")
            else:
            ...
            cursor = conn.cursor()
            cursor.execute(
                "INSERT INTO generations (type, description, status, result) VALUES (?, ?, 'done', ?)",
                (cmd, desc, code)
            )
            conn.commit()
            conn.close()
